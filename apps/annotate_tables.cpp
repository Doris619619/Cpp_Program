...
int main(int argc, char** argv) {
    ...
    if (img.empty()) { std::cerr << "Failed to read: "<<img_path<<"\n"; return 1; }

    std::vector<std::vector<cv::Point>> tables;
    ...
    while (true) {
        ...
        int k = cv::waitKey(30);
        if (k==13 || k==10) { // Enter
            if (current.size()>=3) tables.push_back(current);
            current.clear();
            ...
            for (auto& t : tables) {
                ...
                for (size_t i=0;i<rects.size();++i) {
                    SeatROI s; s.seat_id = (++tid); s.rect = rects[i]; seats.push_back(std::move(s));
                }
            }
